...
const sample_expired_stringified_access_token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhY2Nlc3NfdG9rZW4iLCJ1c2VyX2lkIjo1LCJvYXV0aF90b2tlbiI6Im1vY2tlZF9vYXV0aF90b2tlbiIsImlhdCI6MTYxNDQ2Njk4MywiZXhwIjoxNjE1MDcxNzgzfQ.3_3aR20bB-E9EYrXZrGL663uDLevS4PThkV41EaPMxI';
...
describe('미들웨어에 대한 단위 테스트', () => {
    describe('AccessTokenExtractor 테스트', () => {
        it('엑세스 토큰 객체 발행 성공', async () => {
            ...
            const req = new FakeRequestBuilder().setHeader({ Authorization: sample_stringified_access_token}).build();
            ...
            expect(objectified_token.toString()).toBe(sample_stringified_access_token);
        });
        it('엑세스 토큰 만료', async () => {
            GithubApp.mockImplementation(() => {
                return {
            ...
            const req = new FakeRequestBuilder().setHeader({ Authorization: sample_expired_stringified_access_token}).build();
            const res = new FakeResponse;
        ...
        it('유효하지 않은 엑세스 토큰', async () => {
            ...
            const res = new FakeResponse;
            await expect(AccessTokenExtractor(req, res)).rejects.toThrow(Unauthorized);
        });
    });
});