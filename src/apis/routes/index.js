...
function wrapHttpMethod(express_rotuer, http_method, path, middlewares, callback) {
    if (!callback) {
        callback = middlewares;
        express_rotuer[http_method](path, make_express_controller(callback));
    }
    else {
        middlewares = middlewares.map((middleware) => make_express_middleware(middleware));
        express_rotuer[http_method](path, middlewares, make_express_controller(callback));
    }
}
