...
const DBForUserQuery = {};
const UserQueryBuilder = jest.fn().mockImplementation(() => {
  let { db, code_db } = DBForUserQuery;
  return {
    all: true,
    findAll: function () {
      this.result = db;
...
UserQueryBuilder.mockClear = () => {
  DBForUserQuery.db = [new UserBuilder(1, "Jungwoo-Son").build()];
  DBForUserQuery.scode_db = [
