...
const CodeQueryBuilder = jest.fn()
    .mockImplementation(() => ({
        ...
        delete: function (id) {
            this.cb = (list) => {
                db = db.filter(c => list.reduce((acc, cur) => acc || (cur == c)) == false);
                return list;
            };
        ...
        searchOnTitleAndContent: function(search) {
            this.result = this.result.filter(r => r.title.indexOf(search) != -1 || r.content.indexOf(search) != -1);
            return this;
        },
...
CodeQueryBuilder.mockClear = () => {
    db = [ new CodeBuilder('코드제목', 'rust', 1).setContent('내용').setId(1).setDescription('설명').setCreatedDatetime(new ServiceTime('2021-04-19T00:00:00.000Z')).build() ];
}
